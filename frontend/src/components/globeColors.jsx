/**
 * Color helpers for the Globe component.
 * Pulled out of Globe.jsx so the visual logic can be tweaked
 * or unit-tested without touching the globe.gl wiring.
 */
...
export function getPolygonLabel(country) {
  const name =
    country.properties?.NAME || country.properties?.ADMIN || "Unknown";
  ...
  return `
    ...
    ">
      <strong>${name}</strong>
      <br/>
      <small>Click to explore events</small>
    </div>
  `;
}
...
export function getPointLabel(event) {
  return `
    ...
    ">
      ...
      <br/>
      <small>
        ${event.country || "Global"} · Importance ${event.importance}/10
      </small>
    </div>
  `;
}
